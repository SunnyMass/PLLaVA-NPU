...
class EvalDataset(Dataset):
    ...
    def read_video(self, video_path, bound=None):
        ...
        while current_idx <= max(frame_indices):
            ret, frame = cap.read()
            if not ret:
                break
    ...
    def read_clip_gif(self, video_path, bound=None, fps=25):
        ...
        # return images_group
        raise NotImplementedError("GIF video loading is not supported in this setup.")
...
class ChatPllava:
    ...
    def load_video(self, video_path, num_segments=8, return_msg=False):
        ...
        frame_indices = self.get_index(total_frames, num_segments)

        # 👇 相当于 decord 中 index = linspace(...), batch = get_batch(...)
        second_indices = np.linspace(0, duration, num=int(duration), endpoint=False)
        frame_indices_from_sec = [int(fps * t) for t in second_indices if int(fps * t) < total_frames]

        # 👇 缓存批量帧（可选保留）
        buffer = []
        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
        for frame_idx in frame_indices_from_sec:
        ...
        # 👇 可选信息返回
        if return_msg:
            sec_list = [round(f / fps, 1) for f in frame_indices]
            sec = ", ".join(str(s) for s in sec_list)
            msg = f"The video contains {len(frame_indices)} frames sampled at {sec} seconds."
            return images_group, msg
        else:
            return images_group
