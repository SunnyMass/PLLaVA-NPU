...
def single_test(model, processor, vid_path, num_frames=4, conv_mode="plain"):
    ...
    def get_index(num_frames, num_segments):
        ...
        step = num_frames / float(num_segments)
        return [int(step * i) for i in range(num_segments)]

    def load_video(video_path, num_segments=8, return_msg=False, num_frames=4, resolution=336):
        resize = transforms.Resize(size=(resolution, resolution))
        ...
        for idx in range(total_frames):
            ...
            if idx in frame_indices:
                ...
                images_group.append(resize(img))
...
def main():
    ...
    if result_list is None:
        n_gpus = torch.npu.device_count()  # 或者 torch.npu.device_count()，根据实际硬件
        world_size = n_gpus if n_gpus > 0 else 1

        if world_size > 1:
            logger.info(f'[Multi-GPU] started benchmarking, saving to: {save_path}')
            with mp.Pool(world_size) as pool:
                func = functools.partial(run, args=args, world_size=world_size)
                result_lists = pool.map(func, range(world_size))
                result_list = list(itertools.chain(*result_lists))
        else:
            logger.info('[Single GPU/CPU] Running in single-process mode')
